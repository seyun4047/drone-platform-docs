해당 문서는 gemini-2.5-flash-lite 로 자동 번역되어 부자연스러운 부분이 있을 수 있으니,<br>정확한 내용을 여기서 확인해주세요. : [영어 문서](https://github.com/seyun4047/drone-platform-docs/blob/main/components/trans-tester/trans-tester.md)

---

# 드론 데이터 전송 테스터

---
## 저장소 개요
이 저장소는 **드론 데이터 전송 테스터**를 제공하며,
드론 연결, 텔레메트리 및 이벤트 데이터를 시뮬레이션하여
서버의 API 동작을 검증합니다.
---

## 작동 방식

| 단계 | API 엔드포인트             | 설명 | 목적 | 기타 |
|------|--------------------------|-------------|---------|-----------|
| 1 | `/auth/connect`          | 드론 일련번호 및 장치 이름을 보냅니다. 승인 시 인증 토큰을 수신합니다. | 드론과 서버 간의 유효한 세션 설정 |
| 2 | `/api/telemetry`| 토큰과 함께 정상적인 텔레메트리 데이터 (각도, 위치)를 보냅니다. | 주기적인 드론 상태 정보 전송 | (event=0) |
| 3 | `/api/telemetry` | 이벤트 플래그 및 이벤트 상세 정보 (예: 사람 감지)와 함께 텔레메트리를 보냅니다. | 중요한 감지 이벤트 보고 | (event=1) |
| 4 | `/auth/update`           | 현재 토큰을 보내고 갱신된 토큰을 수신합니다. | 유효한 인증 세션 유지 |
| 5 | `/auth/disconnect`       | 토큰과 함께 연결 해제 요청을 보냅니다. | 연결을 깔끔하게 종료 |

---

## 설치
필요한 종속성을 설치합니다:
```bash
pip install -r requirements.txt
```
---
## 사용법
애플리케이션을 실행합니다:
```bash
python3 main.py
```
---