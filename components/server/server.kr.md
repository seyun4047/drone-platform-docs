...
이 서버는 드론 세션 관리, 인증, 텔레메트리 처리, 그리고 플랫폼 전반의 대량 요청 처리에 대한 전체 수명 주기를 담당합니다.<br>
...
- 접근 제어 강화<br>
인증 및 권한이 부여된 사용자만 대시보드 API에 접근할 수 있도록 하여 데이터 무결성 및 개인 정보 보호를 보장합니다.
...
# 응답: {"status":status,"message":"message","data":{"id":"testid"}}%
```
```bash
# login
...
## AMAZON S3 가이드
### S3 및 IAM 설정 가이드
> S3 가이드를 알고 싶다면 여기를 확인하세요: [S3 GUIDE](https://github.com/seyun4047/drone-platform-docs/blob/main/components/server/S3_GUIDE.md)
---
## 대시보드(프론트엔드) 통신
