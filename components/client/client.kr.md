...
---
## 저장소 개요
- 이 클라이언트 애플리케이션은 드론으로부터 원격 측정 데이터를 수집하여 중앙 서버로 실시간으로 전송합니다.
- 이 시스템은 드론의 비디오 피드와 화면상의 원격 측정 데이터를 지속적으로 모니터링할 수 있는 환경에서 작동하도록 설계되었습니다.
- 일반적으로 원격 측정 정보는 속도 및 기타 비행 데이터가 표시되는 드론 컨트롤러의 디스플레이에서 직접 캡처됩니다.
...
|아무것도 감지되지 않으면, 서버로 원격 측정 데이터를 전송합니다.<br>(이미지는 클라이언트로부터 서버가 수신한 원격 측정 데이터를 나타냅니다.)|
...
드론 원격 측정 데이터는 일반적으로 카메라 드론의 리모컨 화면에서 얻습니다.
화면에는 속도 및 기타 비행 정보가 표시되어야 합니다.
좌표 감지를 위해서는 간단한 GPS 모듈을 부착해야 하며, 그 데이터가 화면에 표시되어 실시간으로 감지될 수 있어야 합니다.
