...
- 일반적으로 원격 측정 정보는 드론 컨트롤러의 디스플레이에서 직접 캡처되며, 이곳에는 속도 및 기타 비행 데이터가 표시됩니다.
...
|승인된 드론 시리얼과 장치 이름을 입력한 후 연결합니다.|화면에서 원하는 정보를 모니터링할 영역을 선택합니다.|데이터가 감지되면 처리되어 메인 서버로 전송됩니다.|
...
|아무것도 감지되지 않으면 서버로 원격 측정 데이터를 전송합니다.<br>(이 이미지는 클라이언트로부터 서버가 수신한 원격 측정 데이터를 나타냅니다.)|
