...
- 일반적으로 원격 측정 정보는 드론 컨트롤러의 디스플레이에서 직접 캡처되며, 여기에는 속도 및 기타 비행 데이터가 표시됩니다.
...
|승인된 드론 일련번호와 장치 이름을 입력한 후 연결합니다.|원하는 정보를 모니터링할 화면 영역을 선택합니다.|데이터가 감지되면 처리되어 메인 서버로 전송됩니다.|
...
드론 원격 측정 데이터는 일반적으로 카메라 드론의 원격 조종기 화면에서 얻어집니다.
