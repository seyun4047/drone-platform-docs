...
- 드론의 영상 피드와 화면상의 텔레메트리 데이터를 지속적으로 모니터링할 수 있는 환경에서 작동하도록 설계되었습니다.
...
|인증된 드론 시리얼 및 장치 이름을 입력하고 연결합니다.|원하는 정보를 모니터링할 화면의 영역을 선택합니다.|데이터가 감지되면 처리되어 메인 서버로 전송됩니다.|
...
좌표 감지를 위해 간단한 GPS 모듈이 연결되어야 하며, 해당 데이터가 화면에 표시되어 실시간으로 감지될 수 있어야 합니다.
