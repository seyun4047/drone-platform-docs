...
|인증된 드론 시리얼과 장치 이름을 입력한 후 연결합니다.|원하는 정보를 모니터링할 화면상의 영역을 선택합니다.|데이터가 감지되면 처리되어 메인 서버로 전송됩니다.|
...
| 카테고리 | 기술 | 버전 | 목적 |
|------------------|---------------------------------|-------------|-------------------------------------|
| Language         | Python                          | 3.x         | 코어 애플리케이션 로직              |
...
좌표 감지를 위해서는 간단한 GPS 모듈을 부착하고, 그 데이터가 화면에 표시되어 실시간으로 감지될 수 있어야 합니다.
