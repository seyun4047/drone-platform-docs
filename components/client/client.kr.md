해당문서는 gemini-2.5-flash-lite 로 번역되어 부자연스러운 부분이 있을 수 있으니, 정확한 내용을 여기서 확인해주세요.₩n₩n원본보기: [ENGLISH GUIDE](https://github.com/seyun4047/drone-platform-docs/blob/main/components/client/client.md)
...
---
## 리포지토리 개요
- 이 클라이언트 애플리케이션은 드론에서 텔레메트리 데이터를 수집하고 실시간으로 중앙 서버로 전송합니다.
- 이 시스템은 드론의 영상 피드 및 화면상의 텔레메트리 데이터를 지속적으로 모니터링할 수 있는 환경에서 작동하도록 설계되었습니다.
- 일반적으로 텔레메트리 정보는 속도 및 기타 비행 데이터가 표시되는 드론 컨트롤러 디스플레이에서 직접 캡처됩니다.
...
|승인된 드론 시리얼 및 장치 이름을 입력한 후 연결합니다.|원하는 정보를 모니터링할 화면 영역을 선택합니다.|데이터가 감지되면 처리되어 메인 서버로 전송됩니다.|
...
|사람을 자동으로 감지합니다.|이벤트 데이터를 서버로 전송합니다.<br>(이미지는 서버에서 클라이언트로부터 받은 이벤트 데이터를 나타냅니다.)|
...
|아무것도 감지되지 않으면 텔레메트리 데이터를 서버로 전송합니다.<br>(이미지는 서버에서 클라이언트로부터 받은 텔레메트리 데이터를 나타냅니다.)|
...
---
## 주의사항
이 시스템은 드론의 영상 정보를 실시간으로 모니터링할 수 있는 환경에서 작동해야 합니다.
드론 텔레메트리 데이터는 일반적으로 카메라 드론의 리모컨 화면에서 얻습니다.
화면에는 속도 및 기타 비행 정보가 표시되어야 합니다.
좌표 탐지를 위해서는 간단한 GPS 모듈이 부착되어야 하며, 그 데이터가 화면에 표시되어 실시간으로 탐지될 수 있어야 합니다.
