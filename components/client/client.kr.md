해당 문서는 gemini-2.5-flash-lite 로 자동 번역되어 부자연스러운 부분이 있을 수 있으니,<br>정확한 내용을 여기서 확인해주세요. : [영어 문서](https://github.com/seyun4047/drone-platform-client/README.md)
...
|승인된 드론 시리얼 및 장치 이름을 입력한 후 연결합니다.|원하는 정보를 모니터링할 화면의 영역을 선택합니다.|데이터가 감지되면 처리되어 메인 서버로 전송됩니다.|
...
| Numeric OCR      | Tesseract (pytesseract)         | 0.3.13      | 속도 및 텔레메트리 숫자 추출 |
...
좌표 탐지를 위해 간단한 GPS 모듈을 부착해야 하며, 해당 데이터는 화면에 표시되어 실시간으로 탐지될 수 있어야 합니다.
