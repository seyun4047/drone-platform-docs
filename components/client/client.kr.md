...
|승인된 드론 시리얼 및 장치 이름을 입력한 후 연결합니다.|원하는 정보를 모니터링할 화면상의 영역을 선택합니다.|데이터가 감지되면 처리되어 메인 서버로 전송됩니다.|
...
|자동으로 사람을 감지합니다.|서버로 이벤트 데이터를 전송합니다.<br>(이미지는 클라이언트로부터 서버가 수신한 이벤트 데이터를 나타냅니다.)|
...
|아무것도 감지되지 않으면 서버로 텔레메트리 데이터를 전송합니다.<br>(이미지는 클라이언트로부터 서버가 수신한 텔레메트리 데이터를 나타냅니다.)|
...
좌표 감지를 위해 간단한 GPS 모듈이 부착되어야 하며, 해당 데이터가 화면에 표시되어 실시간으로 감지될 수 있어야 합니다.